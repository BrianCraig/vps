...
console.log('se quiere quitar');
proceso.kill(hash);
puertos.agregarPuerto(Apps[hash].puerto);
delete Apps[hash];
io.emitir('quitarApp',hash);

};
exports.iniciarApp=function(hash){
	if(!Apps[hash])return 0;
	if(Apps[hash].estado)return 0; // el proceso ya se encuentra activo, PONELE (tal vez hubo un retraso y se cerró hace unos milisegundos )
